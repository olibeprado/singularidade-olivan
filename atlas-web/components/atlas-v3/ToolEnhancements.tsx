    ...
    >
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <span style={{ color: "#e8f1ff", fontSize: 13, fontWeight: 800 }}>⚙ {TOOL_LABELS[local.tool]}</span>
        ...
        {tab === "style" && (
          <div style={{ display: "grid", gap: 12 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => set({ lineStyle: e.target.value as Drawing["lineStyle"] })}
                  style={{ width: "100%", background: "#0a1020", border: "1px solid #1e2d42", borderRadius: 4, color: "#e8f1ff", fontSize: 10, padding: "5px 7px" }}
                >
            ...
            <div>
              <div style={{ fontSize: 9, color: "#7f93b7", marginBottom: 4 }}>Opacidade fundo: {local.fillOpacity}%</div>
              <input
                type="range"
                min={0}
                max={40}
                value={local.fillOpacity}
            ...
            {hasFib && (
              <div style={{ borderTop: "1px solid #172133", paddingTop: 10 }}>
                <div style={{ fontSize: 9, fontWeight: 700, color: "#7f93b7", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Níveis Fibonacci</div>
                <div style={{ maxHeight: 170, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4 }}>
                  {fibLevels.map((lvl, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "auto 1fr auto auto", gap: 4, alignItems: "center" }}>
                      ...
                      />
                      <button
                        onClick={() => set({ fibLevels: fibLevels.filter((_, j) => j !== i) })}
                        style={{ background: "transparent", border: "none", color: "#ff3060", cursor: "pointer", fontSize: 12, padding: "1px 4px" }}
                      >
                        ✕
                      </button>
                    </div>
                ...
                <button
                  onClick={() => set({ fibLevels: [...fibLevels, { pct: 2, color: "#00d4ff", visible: true }] })}
            ...
            {["trendline", "ray", "extended"].includes(local.tool) && (
              <div style={{ borderTop: "1px solid #172133", paddingTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
                <label style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#e8f1ff" }}>
                  Mostrar seta
                  <input type="checkbox" checked={local.showArrow !== false} onChange={(e) => set({ showArrow: e.target.checked })} style={{ accentColor: "#2de2ff" }} />
                </label>
                <label style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#e8f1ff" }}>
                  Variação %
                  <input type="checkbox" checked={!!local.showVariation} onChange={(e) => set({ showVariation: e.target.checked })} style={{ accentColor: "#2de2ff" }} />
                </label>
              </div>
            )}
            {local.tool === "text" && (
              <div style={{ borderTop: "1px solid #172133", paddingTop: 10, display: "grid", gap: 8 }}>
                ...
                <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#e8f1ff" }}>
                  <input type="checkbox" checked={!!local.bold} onChange={(e) => set({ bold: e.target.checked })} style={{ accentColor: "#2de2ff" }} /> Negrito
                </label>
              </div>
            )}
          </div>
        ...
        {tab === "visibility" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <label style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 11, color: "#e8f1ff", padding: "6px 0", borderBottom: "1px solid #172133" }}>
              Visível
              <input type="checkbox" checked={!local.hidden} onChange={(e) => set({ hidden: !e.target.checked })} style={{ accentColor: "#2de2ff", width: 15, height: 15 }} />
            </label>
            <label style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 11, color: "#e8f1ff", padding: "6px 0", borderBottom: "1px solid #172133" }}>
              Travado (não mover)
              <input type="checkbox" checked={local.locked} onChange={(e) => set({ locked: e.target.checked })} style={{ accentColor: "#2de2ff", width: 15, height: 15 }} />
            </label>
            <div>
    ...
    >
      {items.map((item, i) => (
        <React.Fragment key={i}>
          {i === items.length - 1 && <div style={{ height: 1, background: "#172133", margin: "2px 0" }} />}
          <div
            onClick={() => {
              item.action();
              onClose();
...
function useDrawings() {
  ...
  const toggleLock = useCallback(() => {
    ...
    setDrawings((prev) =>
      prev.map((d) => (d.id === selectedId ? { ...d, locked: !d.locked } : d))
    );
  }, [selectedId]);
...
function DrawingOptionsBar({
  ...
  return (
    ...
    >
      <button style={btnStyle(!!selectedDrawing?.locked)} onClick={onToggleLock}>
        🔒 Travar
      </button>
      <button style={btnStyle()} onClick={onOpenSettings}>
        ⚙ Config.
      </button>
      <button style={btnStyle(false, true)} onClick={onDelete}>
        ✕ Apagar
      </button>
  ...
  return (
    <div style={{ height: "100%", background: "linear-gradient(180deg, rgba(6,10,20,0.98), rgba(4,7,15,0.98))", overflowY: "auto" }}>
      ...
      <div style={{ margin: 12, borderRadius: 14, border: "1px solid rgba(255,255,255,0.06)", background: "linear-gradient(180deg, rgba(9,15,29,0.98), rgba(7,12,24,0.98))", padding: 12 }}>
        ...
        {[["Estrutura", insight.structure[0]?.value || "Neutro", ui.green], ["Momentum", insight.structure[1]?.value || "Moderado", "#9fffbc"], ["Confluência", `${Math.max(2, Math.min(9, Math.round(insight.score / 11)))} / 9`, ui.green], ["Razão de Prata", insight.structure2[1]?.value || "Estável", ui.green], ["Ciclo", insight.score >= 75 ? "Acelerado" : "Normal", ui.cyan]].map(([a, b, c]) => (
        ...
        {filtered.map((asset, i) => (
          <div
            ...
            style={{
              ...
              alignItems: "center",
              cursor: "pointer",
              background: asset.symbol === selectedSymbol ? "linear-gradient(90deg, rgba(247,201,72,0.10), rgba(45,226,255,0.06))" : "transparent",
            }}
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column" }}>
              ...
              <span style={{ color: asset.change >= 0 ? ui.green : ui.red, fontSize: 12, fontFamily: "monospace", fontWeight: 800 }}>
                {asset.change >= 0 ? "+" : ""}
...
function MasterScannerPanel({ assets, selectedSymbol, onSelectSymbol }: { assets: AssetScore[]; selectedSymbol: string; onSelectSymbol: (symbol: string) => void }) {
  const [searchTerm, setSearchTerm] = useState("");
  ...
  const toggleFavorite = (symbol: string) => {
    setFavoriteSymbols((prev) => (prev.includes(symbol) ? prev.filter((item) => item !== symbol) : [...prev, symbol]));
  };
  ...
  const marketPositive = assets.filter((item) => item.change >= 0).length / Math.max(1, assets.length) >= 0.6;
  ...
  return (
    <div style={{ height: "100%", borderRadius: 12, border: `1px solid ${ui.border}`, background: "linear-gradient(180deg, rgba(7,10,19,0.98), rgba(5,8,15,0.98))", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "10px 14px", borderBottom: `1px solid ${ui.border}`, display: "grid", gap: 10, flexShrink: 0, background: "radial-gradient(circle at top, rgba(0,216,255,0.08), transparent 38%), linear-gradient(180deg, rgba(10,16,28,0.98), rgba(7,10,19,0.98))" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 220px auto auto", gap: 10, alignItems: "center" }}>
          <div>
            <div style={{ color: "#f2f7ff", fontSize: 18, fontWeight: 900, letterSpacing: 0.5 }}>MESTRE SCANNER</div>
            <div style={{ color: "#7f93b7", fontSize: 11 }}>Dominância BTC em evidência, BTC fixo e ordenação por preço ou variação.</div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, height: 36, padding: "0 12px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.03)" }}>
            <Search size={14} color="#8ca0c6" />
            <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Buscar ativo..." style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: "#e9f3ff", fontSize: 12 }} />
          </div>
          <button onClick={() => setShowFavoritesOnly(false)} style={{ height: 34, padding: "0 12px", borderRadius: 10, border: !showFavoritesOnly ? "1px solid rgba(45,226,255,0.25)" : "1px solid rgba(255,255,255,0.06)", background: !showFavoritesOnly ? "linear-gradient(180deg, rgba(45,226,255,0.14), rgba(45,226,255,0.04))" : "rgba(255,255,255,0.03)", color: !showFavoritesOnly ? ui.cyan : "#dce8ff", fontSize: 11, fontWeight: 900, cursor: "pointer" }}>
            Mercado
          </button>
          <button onClick={() => setShowFavoritesOnly(true)} style={{ height: 34, padding: "0 12px", borderRadius: 10, border: showFavoritesOnly ? "1px solid rgba(247,201,72,0.3)" : "1px solid rgba(255,255,255,0.06)", background: showFavoritesOnly ? "linear-gradient(180deg, rgba(247,201,72,0.15), rgba(247,201,72,0.04))" : "rgba(255,255,255,0.03)", color: showFavoritesOnly ? ui.yellow : "#dce8ff", fontSize: 11, fontWeight: 900, cursor: "pointer" }}>
            Favoritos
          </button>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 10 }}>
          <SmallStatCard title="Dominância BTC" value="52.43%" sub="BTC liderando o fluxo macro." color={ui.yellow} accent="rgba(247,201,72,0.12)" />
          <SmallStatCard title="BTC Fixo" value="$74.682" sub="Sempre visível no topo do scanner." color={ui.cyan} accent="rgba(45,226,255,0.1)" />
          <SmallStatCard title="Favoritos" value={`${favoriteSymbols.length}`} sub="Lista rápida com seus ativos-chave." color={ui.green} accent="rgba(39,245,157,0.1)" />
          <SmallStatCard title="Mercado" value={marketPositive ? "Bull Bias" : "Cautela"} sub={marketPositive ? "Mais ativos subindo do que caindo." : "Mercado misto e seletivo."} color={marketPositive ? ui.green : ui.red} accent={marketPositive ? "rgba(39,245,157,0.1)" : "rgba(255,107,134,0.08)"} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.9fr 0.95fr 0.9fr 0.8fr 1fr", gap: 10, color: "#6c7da2", fontSize: 11, alignItems: "center" }}>
          <span>Ativo</span>
          <span>Top Forge</span>
          <button onClick={() => toggleSort("price")} style={{ background: "transparent", border: "none", padding: 0, color: sortKey === "price" ? ui.cyan : "#6c7da2", fontSize: 11, fontWeight: 800, cursor: "pointer", textAlign: "left" }}>
            Preço {sortKey === "price" ? (sortDir === "desc" ? "↓" : "↑") : ""}
          </button>
          <button onClick={() => toggleSort("change")} style={{ background: "transparent", border: "none", padding: 0, color: sortKey === "change" ? ui.cyan : "#6c7da2", fontSize: 11, fontWeight: 800, cursor: "pointer", textAlign: "left" }}>
            % 24h {sortKey === "change" ? (sortDir === "desc" ? "↓" : "↑") : ""}
          </button>
          <button onClick={() => toggleSort("dominance")} style={{ background: "transparent", border: "none", padding: 0, color: sortKey === "dominance" ? ui.cyan : "#6c7da2", fontSize: 11, fontWeight: 800, cursor: "pointer", textAlign: "left" }}>
            Dom. BTC {sortKey === "dominance" ? (sortDir === "desc" ? "↓" : "↑") : ""}
          </button>
          <span>Mini Chart</span>
        </div>
      </div>
      <div data-atlas-scroll="cyan" style={{ flex: 1, overflowY: "auto", padding: "0 0 4px" }}>
          ...
          return (
            <div
              ...
              style={{
                ...
                alignItems: "center",
                cursor: "pointer",
                background: isBtc ? "linear-gradient(90deg, rgba(247,201,72,0.14), rgba(45,226,255,0.08))" : asset.symbol === selectedSymbol ? "linear-gradient(90deg, rgba(45,226,255,0.08), rgba(255,255,255,0.01))" : "transparent",
              }}
            >
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <button
                ...
                <div style={{ display: "grid", gap: 2 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ width: 8, height: 8, borderRadius: "50%", background: asset.color, display: "inline-block", boxShadow: `0 0 12px ${asset.color}` }} />
                    <span style={{ color: "#edf5ff", fontSize: 13, fontWeight: 900 }}>{asset.symbol}</span>
                    {isBtc && <span style={{ padding: "3px 7px", borderRadius: 999, background: "rgba(247,201,72,0.14)", color: ui.yellow, fontSize: 8, fontWeight: 900 }}>FIXO</span>}
                  </div>
                  <span style={{ color: "#6c7da2", fontSize: 10 }}>{isBtc ? "Referência principal do scanner." : "Ordenação dinâmica por mercado."}</span>
                </div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <div style={{ color: scoreVisual.color, fontSize: 12, fontWeight: 900 }}>{asset.signal}</div>
                <ScoreBar value={asset.aiScore} />
              </div>
              <div style={{ display: "flex", flexDirection: "column" }}>
                <span style={{ color: "#eef5ff", fontSize: 12, fontFamily: "monospace" }}>${asset.price.toLocaleString()}</span>
                <span style={{ color: "#7f93b7", fontSize: 10 }}>{asset.riskType}</span>
              </div>
              <div style={{ color: asset.change >= 0 ? ui.green : ui.red, fontSize: 12, fontFamily: "monospace", fontWeight: 900 }}>
                {asset.change >= 0 ? "+" : ""}
                {asset.change.toFixed(2)}%
              </div>
              <div style={{ color: isBtc ? ui.yellow : "#dce8ff", fontSize: 12, fontFamily: "monospace", fontWeight: 900 }}>{dominance.toFixed(2)}%</div>
  ...
  return (
    <div
      style={{
        ...
        background: "linear-gradient(180deg, rgba(6,13,24,0.98), rgba(4,8,16,0.98))",
...
function LiquidityPanel() {
  ...
  const renderMain = () => {
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                {["1M", "5M", "15M", "1H", "4H", "1D"].map((tf) => (
                  <button key={tf} style={{ height: 22, minWidth: 28, borderRadius: 6, border: tf === "1H" ? "1px solid rgba(45,226,255,0.55)" : "1px solid rgba(255,255,255,0.08)", background: tf === "1H" ? "rgba(45,226,255,0.14)" : "rgba(255,255,255,0.02)", color: tf === "1H" ? "#dffcff" : "#7f95bb", fontSize: 9, fontWeight: 900, padding: "0 8px" }}>
                    {tf}
                  </button>
                ))}
              </div>
      ...
      return (
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <div style={{ height: 320, borderRadius: 14, border: "1px solid rgba(255,255,255,0.05)", background: "linear-gradient(180deg, rgba(6,13,24,0.96), rgba(5,9,18,0.98))", position: "relative" }}>
            {[["BTC Core", 180, 165, 96, "#8b5cf6"], ["ETH Core", 360, 105, 70, "#2de2ff"], ["BTC Alta", 120, 285, 52, "#27f59d"], ["Whale Sell", 420, 260, 46, "#ff4d8d"], ["Retail", 300, 305, 34, "#ff944d"]].map(([name, x, y, size, color]) => (
    ...
    if (tab === "Eventos") {
      return (
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {[["14:32", "LIQUIDAÇÃO", "$71.265", "Liquidação em massa shorts detectada", ui.green], ["14:28", "WHALE", "$70.980", "Ordem de compra institucional +$12.4M", ui.green], ["14:21", "CLUSTER", "$71.100", "BTC Core absorvendo pressão vendedora", ui.cyan], ["14:15", "PRESSÃO", "$70.750", "Pressão vendedora concentrada em $70.800", ui.red], ["14:08", "SUPORTE", "$71.420", "Suporte forte confirmado em $71.200", ui.green]].map(([time, label, price, desc, color]) => (
        ...
        <div style={{ display: "grid", gap: 10, alignContent: "start", marginTop: -2 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div style={{ borderRadius: 14, border: `1px solid ${ui.green}55`, background: "rgba(10, 31, 24, 0.55)", padding: 14 }}>
              ...
              <div style={{ height: 8, borderRadius: 999, background: "rgba(255,255,255,0.08)", overflow: "hidden", marginBottom: 10 }}>
                <div style={{ width: "93%", height: "100%", background: ui.green }} />
              </div>
            ...
            <div style={{ borderRadius: 14, border: `1px solid ${ui.red}55`, background: "rgba(31, 10, 20, 0.55)", padding: 14 }}>
              ...
              <div style={{ height: 8, borderRadius: 999, background: "rgba(255,255,255,0.08)", overflow: "hidden", marginBottom: 10 }}>
                <div style={{ width: "7%", height: "100%", background: ui.red }} />
              </div>
              <div style={{ display: "grid", gap: 6 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "0.74fr 0.92fr 0.74fr", gap: 10, alignItems: "stretch" }}>
              ...
              {[["🦈", "Tubarões", "-125M", false], ["🏛️", "Institucionais", "+582M", true], ["🐟", "Sardinhas", "+320M", true]].map(([icon, name, val, pos]) => (
                <div key={String(name)} style={{ borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)", padding: 12, background: "rgba(255,255,255,0.015)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <span style={{ color: "#edf5ff", fontSize: 14, fontWeight: 900, display: "flex", gap: 8, alignItems: "center" }}>
                      <span>{icon}</span>
                      {name}
                    </span>
            ...
            <div style={{ borderRadius: 14, border: "1px solid rgba(255,255,255,0.06)", background: "linear-gradient(180deg, rgba(9,15,29,0.98), rgba(7,12,24,0.98))", padding: 12 }}>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                {[["$87.026", "0.666", "$417K", ui.red], ["$85.869", "6.538", "$471K", ui.red], ["$87.886", "4.878", "$284K", ui.red], ["$87.627", "6.447", "$413K", ui.red], ["$87.318", "10.774", "$279K", ui.red], ["$87.112", "10.515", "$365K", ui.green], ["$87.118", "11.838", "$194K", ui.green], ["$86.715", "10.027", "$280K", ui.green], ["$87.047", "11.168", "$70K", ui.green], ["$86.688", "2.729", "$321K", ui.green]].map(([price, qty, total, c], idx) => (
    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {[["Negativo", "Fed mantém taxa de juros em 5,25% - 5,50%", ui.red], ["Positivo", "BlackRock adiciona mais $2.1B em Bitcoin ETF", ui.green], ["Positivo", "Binance anuncia suporte para Ethereum Shanghai", ui.green], ["Neutro", "Regulação crypto na Europa: MiCA entra em vigor", ui.yellow]].map(([tone, title, color]) => (
...
function ChartPanel({
  ...
  const handleOverlayMouseUp = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (draftStart && drawingState.activeTool !== "cursor") {
      ...
      if (distance > 5) {
        const newDraw = newDrawing(drawingState.activeTool, draftStart.x, draftStart.y, p.x, p.y);
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr repeat(4, 0.7fr) auto", gap: 8, alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
            ...
            <div>
              ...
              <div style={{ color: "#7d91b6", fontSize: 10, fontWeight: 700 }}>Scanner Atlas • Ferramenta: {toolLabelMap[drawingState.activeTool]} • TF: {timeframe}</div>
            </div>
          </div>
          {[["Preço", livePrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }), "#4ef0cb"], ["Variação", `${isPositive ? "+" : ""}${priceChange.toFixed(2)}%`, isPositive ? ui.green : ui.red], ["Volume", formatCompact(candles[candles.length - 1]?.volume ?? 0), ui.cyan], ["Desenhos", String(drawingState.drawings.filter((d) => !d.hidden).length), drawingState.drawings.length > 0 ? ui.yellow : ui.red]].map(([title, value, color]) => (
      ...
      <div style={{ position: "relative", flex: 1, minHeight: 0 }}>
        ...
        >
          {drawingState.drawings.filter((d) => !d.hidden).map((d) => (
            <g key={d.id}>{renderDrawingSVG(d, svgSize.w, svgSize.h, d.id === drawingState.selectedId)}</g>
          ))}
        </svg>
...
function EulerModule({ insight }: { insight: AIInsight }) {
  ...
  return (
    <div style={{ height: "100%", padding: 10, background: "linear-gradient(180deg, rgba(5,8,16,0.98), rgba(2,6,14,0.98))" }}>
      <div style={{ height: "100%", borderRadius: 14, border: "1px solid rgba(45,226,255,0.12)", background: "linear-gradient(180deg, rgba(4,10,20,0.98), rgba(2,6,12,0.98))", overflow: "hidden", display: "grid", gridTemplateRows: "auto auto 1fr auto", gap: 10, padding: 10, boxShadow: "0 0 30px rgba(45,226,255,0.08)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "4px 4px 0" }}>
          <div>
            <div style={{ color: "#eef6ff", fontSize: 15, fontWeight: 900, letterSpacing: 0.6 }}>Euler Quantum Interface</div>
            <div style={{ color: "#6f88af", fontSize: 10, fontWeight: 800, letterSpacing: 0.5, textTransform: "uppercase" }}>Scientific module • atlas sync active</div>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <TopButton active>Σ Fórmula</TopButton>
            <TopButton>Sync</TopButton>
            <span style={{ color: ui.cyan, fontSize: 10, fontWeight: 900, padding: "4px 8px", borderRadius: 999, background: "rgba(45,226,255,0.10)", border: "1px solid rgba(45,226,255,0.18)" }}>Módulo Ativo</span>
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.28fr 0.72fr", gap: 10, minHeight: 0 }}>
          <div style={{ borderRadius: 14, border: "1px solid rgba(45,226,255,0.12)", background: "radial-gradient(circle at 50% 0%, rgba(45,226,255,0.08), transparent 35%), linear-gradient(180deg, rgba(6,12,24,0.98), rgba(3,7,14,0.98))", overflow: "hidden" }}>
            <div style={{ padding: "12px 14px 8px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ color: "#6f88af", fontSize: 10, textTransform: "uppercase", letterSpacing: 0.5 }}>{insight.symbol} • previsão estrutural</div>
              </div>
            ...
            <div style={{ padding: 12, height: "calc(100% - 58px)" }}>
              <svg width="100%" height="100%" viewBox="0 0 860 330" preserveAspectRatio="none">
                ...
                {bars.map((h, i) => {
                  const x = 26 + i * 56;
                  return <rect key={i} x={x} y={300 - h} width="22" height={h} rx="4" fill={i % 2 === 0 ? "rgba(45,226,255,0.38)" : "rgba(247,201,72,0.28)"} />;
                })}
          ...
          <div style={{ borderRadius: 14, border: "1px solid rgba(45,226,255,0.12)", background: "linear-gradient(180deg, rgba(7,12,24,0.98), rgba(3,7,14,0.98))", padding: 12 }}>
            <div style={{ color: "#edf6ff", fontSize: 13, fontWeight: 900, marginBottom: 10 }}>Leitura do Módulo</div>
            <div style={{ display: "grid", gap: 6 }}>
              {[["Ativo", insight.symbol, "#dce8ff"], ["Score", `${insight.score}`, ui.green], ["Sinal", insight.signal, ui.yellow], ["Risco", insight.riskLevel, ui.red], ["Liquidez", insight.structure[2]?.value || "Médio", ui.cyan], ["Euler", insight.structure2[0]?.value || "Estável", ui.green]].map(([k, v, c]) => (
            ...
            <div style={{ marginTop: 14, borderRadius: "50%", width: 120, height: 120, marginInline: "auto", display: "flex", alignItems: "center", justifyContent: "center", background: "radial-gradient(circle at 50% 50%, rgba(45,226,255,0.22), rgba(45,226,255,0.05) 55%, transparent 72%)", border: "1px solid rgba(45,226,255,0.16)" }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ color: ui.cyan, fontSize: 28, fontWeight: 900 }}>{insight.score}</div>
                <div style={{ color: "#7f93b7", fontSize: 10, fontWeight: 800, letterSpacing: 0.8, textTransform: "uppercase", marginTop: 4 }}>Sincronia</div>
              </div>
            </div>
          </div>
        </div>
...
function SingularidadeModule({ insight }: { insight: AIInsight }) {
  ...
  const liveList = [["BTC", "$66,374.82", "+2.76%"], ["ETH", "$3,931.95", "+2.58%"], ["SOL", "$174.80", "+3.06%"], ["ADA", "$0.6186", "+5.05%"], ["DOT", "$8.4004", "+0.35%"]];
  ...
  return (
    <div style={{ height: "100%", padding: 10, background: "linear-gradient(180deg, rgba(4,7,14,0.98), rgba(2,5,10,0.98))" }}>
      <div style={{ height: "100%", borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(4,8,18,0.98), rgba(2,5,10,0.98))", overflow: "hidden", display: "grid", gridTemplateRows: "auto auto 1fr auto", gap: 10, padding: 10, position: "relative" }}>
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none", background: "linear-gradient(rgba(0,214,255,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(0,214,255,0.02) 1px, transparent 1px)", backgroundSize: "28px 28px", opacity: 0.35 }} />
        <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "2px 4px 0" }}>
          <div>
            ...
            <div style={{ color: "#6883aa", fontSize: 10, fontWeight: 800, letterSpacing: 0.5, textTransform: "uppercase" }}>Quantum state monitor</div>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <span style={{ color: "#00ff9d", fontSize: 10, fontWeight: 900, letterSpacing: 0.7, textTransform: "uppercase" }}>• Ao vivo</span>
            <span style={{ color: ui.cyan, fontSize: 10, fontWeight: 900, padding: "4px 8px", borderRadius: 999, background: "rgba(0,214,255,0.08)", border: "1px solid rgba(0,214,255,0.16)" }}>Módulo Ativo</span>
          </div>
        </div>
        ...
        <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "1.24fr 0.76fr", gap: 10, minHeight: 0 }}>
          <div style={{ display: "grid", gridTemplateRows: "1fr auto", gap: 10, minHeight: 0 }}>
            <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", overflow: "hidden", minHeight: 0 }}>
              <div style={{ padding: "12px 14px 8px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ color: "#eef7ff", fontSize: 13, fontWeight: 900 }}>Singularidade</span>
                  <span style={{ color: ui.magenta, fontSize: 10, fontWeight: 900, padding: "3px 8px", borderRadius: 999, background: "rgba(255,79,163,0.10)", border: "1px solid rgba(255,79,163,0.16)" }}>{insight.price.toLocaleString()}</span>
                </div>
                <span style={{ color: ui.cyan, fontSize: 10, fontWeight: 900, padding: "4px 10px", borderRadius: 999, background: "rgba(0,214,255,0.08)", border: "1px solid rgba(0,214,255,0.16)" }}>Atlas Sync</span>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "0.86fr 1fr 1fr", gap: 10 }}>
              <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ textAlign: "center" }}>
                  <div style={{ color: "#6f88af", fontSize: 10, fontWeight: 900, letterSpacing: 0.7, textTransform: "uppercase", marginBottom: 14 }}>Índice Singular</div>
                  <div style={{ width: 108, height: 108, borderRadius: "50%", margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "center", background: "conic-gradient(#00ff9d 0deg, #00ff9d 290deg, rgba(255,255,255,0.08) 290deg 360deg)", padding: 8 }}>
                    <div style={{ width: "100%", height: "100%", borderRadius: "50%", background: "#06101b", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }}>
              ...
              <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 14 }}>
                <div style={{ color: "#edf7ff", fontSize: 13, fontWeight: 900, marginBottom: 12 }}>Mercado Live</div>
                <div style={{ display: "grid", gap: 10 }}>
                  {liveList.map(([sym, price, change]) => (
                    <div key={sym} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, paddingBottom: 8, borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      <div>
              ...
              <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 14, display: "grid", alignContent: "start", gap: 10 }}>
                <div style={{ color: "#edf7ff", fontSize: 13, fontWeight: 900 }}>Métricas</div>
                {[["Momentum", "78.4%", ui.cyan], ["Proteção", "92%", "#00ff9d"], ["Volatilidade", "34.2", ui.yellow], ["Desvio", "1.82", ui.magenta]].map(([k, v, c]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                    <span style={{ color: "#7f93b7", fontSize: 13 }}>{k}</span>
                    <span style={{ color: c as string, fontSize: 13, fontWeight: 900 }}>{v}</span>
                  </div>
          ...
          <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 12, overflowY: "auto" }}>
            <div style={{ color: "#edf7ff", fontSize: 13, fontWeight: 900, marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.7 }}>Leitura do módulo</div>
            <div style={{ borderRadius: 12, border: "1px solid rgba(0,214,255,0.10)", background: "linear-gradient(180deg, rgba(6,14,24,0.98), rgba(5,10,20,0.98))", padding: 14, marginBottom: 12, minHeight: 108, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ color: ui.magenta, fontSize: 32, fontWeight: 900 }}>{Math.max(4, Math.round(insight.score / 20))}</div>
                <div style={{ color: "#7f93b7", fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: 0.7, marginTop: 6 }}>Score</div>
              </div>
            </div>
            <div style={{ display: "grid", gap: 6 }}>
              {[["Ativo", insight.symbol, "#dce8ff"], ["Sinal", singSignal, singSignalColor], ["Risco", "Alto", ui.red], ["Liquidez", "Inativo", "#8b97ad"], ["Euler", "Desalinhado", ui.magenta]].map(([k, v, c]) => (
            ...
            <div style={{ marginTop: 14, borderRadius: 12, height: 82, border: "1px solid rgba(255,255,255,0.05)", background: "radial-gradient(circle at 50% 50%, rgba(255,79,163,0.12), rgba(0,214,255,0.06), transparent 70%)", display: "flex", alignItems: "center", justifyContent: "center", color: "#647da3", fontSize: 10, textTransform: "uppercase", letterSpacing: 1 }}>Campo quântico ativo</div>
          </div>
        </div>
        <div style={{ position: "relative", zIndex: 1, height: 40, borderRadius: 10, border: "1px solid rgba(0,214,255,0.10)", background: "rgba(0,0,0,0.28)", display: "flex", alignItems: "center", overflow: "hidden", padding: "0 12px", gap: 18 }}>
          {[["ETH", "$3,482", "+1.12%", "#00ff9d"], ["SOL", "$187.40", "-0.87%", "#ff4fa3"], ["BNB", "$612.38", "+0.43%", "#00ff9d"], ["ADA", "$0.847", "+3.21%", "#00ff9d"], ["DOT", "$9.34", "-1.05%", "#ff4fa3"], ["AVAX", "$42.18", "+4.78%", "#00ff9d"], ["MATIC", "$1.23", "+0.93%", "#00ff9d"]].map(([sym, price, change, color]) => (
            <div key={sym} style={{ display: "flex", alignItems: "center", gap: 8, whiteSpace: "nowrap", fontSize: 11 }}>
...
function IAAtlasModule({ insight }: { insight: AIInsight }) {
  ...
  return (
    <div style={{ height: "100%", padding: 10, background: "linear-gradient(180deg, rgba(4,8,16,0.98), rgba(2,6,12,0.98))" }}>
      <div style={{ height: "100%", borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(4,10,20,0.98), rgba(2,6,12,0.98))", display: "grid", gridTemplateRows: "auto auto 1fr auto", gap: 10, padding: 10, overflow: "hidden", position: "relative" }}>
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none", background: "linear-gradient(rgba(0,214,255,0.018) 1px, transparent 1px), linear-gradient(90deg, rgba(0,214,255,0.018) 1px, transparent 1px)", backgroundSize: "28px 28px", opacity: 0.35 }} />
        <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ color: "#eaf6ff", fontSize: 15, fontWeight: 900, letterSpacing: 0.8 }}>IA ATLAS</div>
            <div style={{ color: "#6883aa", fontSize: 10, fontWeight: 800, letterSpacing: 0.5, textTransform: "uppercase" }}>Sistema preditivo de ativos digitais</div>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <TopButton active>Atlas Sync</TopButton>
            <span style={{ color: "#00ff9d", fontSize: 10, fontWeight: 900, letterSpacing: 0.7, textTransform: "uppercase" }}>• Online</span>
          </div>
        </div>
        <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "0.8fr 1.3fr 1fr", gap: 10 }}>
          <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 12, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <div style={{ textAlign: "center" }}>
              <div style={{ width: 104, height: 104, borderRadius: "50%", margin: "0 auto 10px", display: "flex", alignItems: "center", justifyContent: "center", background: "conic-gradient(#00ff9d 0deg, #24f5d6 300deg, rgba(255,255,255,0.08) 300deg 360deg)", padding: 8 }}>
                <div style={{ width: "100%", height: "100%", borderRadius: "50%", background: "#06101b", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }}>
                  <span style={{ color: "#00ff9d", fontSize: 20, fontWeight: 900 }}>{insight.score}</span>
                  <span style={{ color: "#7f93b7", fontSize: 10 }}>Forte</span>
                </div>
              </div>
              <div style={{ color: "#7f93b7", fontSize: 10, textTransform: "uppercase", letterSpacing: 0.7 }}>Score Atlas</div>
            </div>
          </div>
          <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 12, display: "grid", gap: 10 }}>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
          ...
          <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 12 }}>
            <div style={{ color: "#edf7ff", fontSize: 13, fontWeight: 900, marginBottom: 10 }}>Sentimento do Mercado</div>
            <div style={{ display: "grid", gap: 10 }}>
        ...
        <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: 10, minHeight: 0 }}>
          <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", overflow: "hidden" }}>
            <div style={{ padding: "12px 14px 8px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
          ...
          <div style={{ display: "grid", gridTemplateRows: "1fr 1fr", gap: 10 }}>
            <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 14 }}>
              <div style={{ color: "#00ff9d", fontSize: 13, fontWeight: 900, marginBottom: 10 }}>Probabilidade Alta</div>
              <div style={{ color: "#00ff9d", fontSize: 42, fontWeight: 900, marginBottom: 10 }}>{probUp.toFixed(1)}%</div>
            ...
            <div style={{ borderRadius: 14, border: "1px solid rgba(255,79,163,0.16)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 14 }}>
              <div style={{ color: ui.magenta, fontSize: 13, fontWeight: 900, marginBottom: 10 }}>Probabilidade Baixa</div>
              <div style={{ color: ui.magenta, fontSize: 42, fontWeight: 900, marginBottom: 10 }}>{probDown.toFixed(1)}%</div>
              <div style={{ height: 8, borderRadius: 999, background: "rgba(255,255,255,0.06)", overflow: "hidden", marginBottom: 12 }}>
        ...
        <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 10 }}>
          <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 14 }}>
            <div style={{ color: "#edf7ff", fontSize: 13, fontWeight: 900, marginBottom: 12 }}>Módulo Atlas</div>
            <div style={{ display: "grid", gap: 8 }}>
              {[["Ativo", insight.symbol, "#dce8ff"], ["Risco", insight.riskLevel, ui.yellow], ["Liquidez", "Ativo", ui.green], ["Euler", "Alinhado", ui.green]].map(([k, v, c]) => (
                <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                  <span style={{ color: "#7f93b7", fontSize: 13 }}>{k}</span>
                  <span style={{ color: c as string, fontSize: 13, fontWeight: 900 }}>{v}</span>
                </div>
          ...
          <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 14 }}>
            <div style={{ color: "#edf7ff", fontSize: 13, fontWeight: 900, marginBottom: 12 }}>Previsão Temporal IA</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
              {[["1H", "55.0%", "30.0%", "15%"], ["4H", "62.0%", "25.0%", "13%"], ["1D", "73.0%", "19.0%", "8%"], ["1W", "68.0%", "22.0%", "10%"]].map(([tf, up, down, neutral]) => (
...
function EstruturaModule({ insight }: { insight: AIInsight }) {
  ...
  return (
    <div style={{ height: "100%", padding: 10, background: "linear-gradient(180deg, rgba(4,8,16,0.98), rgba(2,6,12,0.98))" }}>
      <div style={{ height: "100%", borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(4,10,20,0.98), rgba(2,6,12,0.98))", overflow: "hidden", display: "grid", gridTemplateRows: "auto 1fr", gap: 10, padding: 10, position: "relative" }}>
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none", background: "linear-gradient(rgba(0,214,255,0.018) 1px, transparent 1px), linear-gradient(90deg, rgba(0,214,255,0.018) 1px, transparent 1px)", backgroundSize: "28px 28px", opacity: 0.35 }} />
        <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
        ...
        <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateRows: "1fr auto auto", gap: 10, minHeight: 0 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1.18fr 0.82fr", gap: 10, minHeight: 0 }}>
            ...
            <div style={{ borderRadius: 14, border: "1px solid rgba(0,214,255,0.12)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 12 }}>
              <div style={{ color: "#edf7ff", fontSize: 13, fontWeight: 900, marginBottom: 12 }}>Leitura do Módulo</div>
              <div style={{ width: 110, height: 56, margin: "0 auto 14px", borderTopLeftRadius: 110, borderTopRightRadius: 110, border: "8px solid rgba(255,255,255,0.08)", borderBottom: "none", position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", inset: 0, borderTopLeftRadius: 110, borderTopRightRadius: 110, border: `8px solid ${ui.green}`, borderBottom: "none", clipPath: "inset(0 35% 0 0)" }} />
                <div style={{ position: "absolute", bottom: -4, left: "50%", transform: "translateX(-50%)", color: ui.green, fontWeight: 900 }}>{insight.score}</div>
              </div>
              <div style={{ display: "grid", gap: 6 }}>
                {[["Ativo", insight.symbol, "#dce8ff"], ["Score", `${insight.score}`, ui.green], ["Sinal", "Compra Forte", ui.green], ["Risco", "Baixo", ui.green], ["Liquidez", "Ativo", ui.green], ["Euler", "Alinhado", ui.cyan]].map(([k, v, c]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                    <span style={{ color: "#7f93b7", fontSize: 13 }}>{k}</span>
                    <span style={{ color: c as string, fontSize: 13, fontWeight: 900 }}>{v}</span>
                  </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div style={{ borderRadius: 14, border: "1px solid rgba(0,255,157,0.16)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 14 }}>
              <div style={{ color: "#00ff9d", fontSize: 13, fontWeight: 900, marginBottom: 12 }}>Cenário Alta</div>
              <div style={{ height: 8, borderRadius: 999, background: "rgba(255,255,255,0.06)", overflow: "hidden", marginBottom: 12 }}>
                <div style={{ width: "86%", height: "100%", background: "#00ff9d" }} />
              </div>
              <div style={{ display: "grid", gap: 8 }}>
                {[["Alvo 1", "$92,812"], ["Alvo 2", "$93,513"], ["Stop Loss", "$84,711"]].map(([k, v]) => (
            ...
            <div style={{ borderRadius: 14, border: "1px solid rgba(255,79,163,0.16)", background: "linear-gradient(180deg, rgba(5,11,22,0.98), rgba(3,7,14,0.98))", padding: 14 }}>
              <div style={{ color: ui.magenta, fontSize: 13, fontWeight: 900, marginBottom: 12 }}>Cenário Baixa</div>
              <div style={{ height: 8, borderRadius: 999, background: "rgba(255,255,255,0.06)", overflow: "hidden", marginBottom: 12 }}>
                <div style={{ width: "14%", height: "100%", background: ui.magenta }} />
              </div>
              <div style={{ display: "grid", gap: 8 }}>
                {[["Alvo 1", "$83,451"], ["Alvo 2", "$81,301"], ["Stop Loss", "$85,257"]].map(([k, v]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ color: "#7f93b7", fontSize: 13 }}>{k}</span>
                    <span style={{ color: k === "Stop Loss" ? ui.yellow : "#dce8ff", fontSize: 13, fontWeight: 900 }}>{v}</span>
                  </div>
...
function WorkspaceByModule({
  ...
  if (activeModule === "Scanner") {
    return <ChartPanel candles={candles} indicators={indicators} selectedObject={selectedObject} mode={mode} symbol={symbol} timeframe={timeframe} selectedTool={selectedTool} drawingState={drawingState} onContextMenu={onContextMenu} onDoubleClick={onDoubleClick} onSetSettingsDrawing={onSetSettingsDrawing} />;
  }
  if (activeModule === "Mestre Scanner") {
    return (
      <div style={{ height: "100%", padding: 10 }}>
        <MasterScannerPanel assets={scannerAssets} selectedSymbol={symbol} onSelectSymbol={onSelectSymbol} />
      </div>
    );
  }
...
export default function AtlasChartPro2() {
  ...
  return (
    <div style={{ width: "100%", height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden", background: ui.bg, color: ui.text, fontFamily: "Inter, Arial, sans-serif" }}>
        ...
        <div style={{ flex: 1, minWidth: 0, minHeight: 0 }}>
          <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 320px", height: "100%", minHeight: 0 }}>
            <div style={{ minWidth: 0, minHeight: 0 }}>
              <WorkspaceByModule activeModule={activeModule} candles={candles} indicators={indicators} selectedObject={selectedObject} mode={mode} symbol={activeAsset.symbol} timeframe={timeframe} events={scannerEvents} insight={insight} scannerAssets={scannerAssets} onSelectSymbol={setSelectedSymbol} selectedTool={drawingState.activeTool} drawingState={drawingState} onContextMenu={(drawing, x, y) => setContextMenu({ x, y, drawing })} onDoubleClick={(drawing) => setSettingsDrawing(drawing)} onSetSettingsDrawing={setSettingsDrawing} />
            </div>
      ...
      {settingsDrawing && <DrawingSettingsModal drawing={settingsDrawing} onApply={(updated) => { drawingState.applySettings(updated); setSettingsDrawing(null); }} onClose={() => setSettingsDrawing(null)} />}
    </div>
  );
}
