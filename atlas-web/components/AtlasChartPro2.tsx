            ...
            {active.items.map((item) => (
              <button
                ...
                style={{
                  ...
                  border: "none",
                  background:
                    item.id === currentTool ? "rgba(45,226,255,0.08)" : "transparent",
              ...
              >
                <span
                  style={{
                    color: item.id === currentTool ? ui.cyan : "#7e90b4",
                    display: "flex",
                    alignItems: "center",
                  }}
                >
...
// ============================================================
// CHART PANEL - COM DESENHOS FUNCIONAIS!
// ============================================================
function ChartPanel({
  candles,
  drawings,
  ...
  const [renderTick, setRenderTick] = useState(0); // ✅ FORÇA RE-RENDER
    ...
    switch (tool) {
      case "trendline":
        return p2
          ? {
              id,
              name: `Trend Line ${drawings.length + 1}`,
              type: "trendline",
              p1,
              p2,
              locked: false,
      ...
      case "ray":
        return p2
          ? { id, name: `Ray ${drawings.length + 1}`, type: "ray", p1, p2, locked: false, hidden: false }
          : null;
      case "hline":
        return {
          id,
  ...
  // ✅ HIT TEST COM PAD = 12 (ERA 8 - MUITO DIFÍCIL DE CLICAR)
  const hitTest = (clientX: number, clientY: number) => {
    const rect = mainRef.current?.getBoundingClientRect();
    if (!rect) return null;
    const x = clientX - rect.left;
    const y = clientY - rect.top;
    const PAD = 12; // ✅ AUMENTADO DE 8 PARA 12
    ...
    for (let i = drawings.length - 1; i >= 0; i--) {
      ...
      if (obj.type === "hline") {
        ...
        if (Math.abs(y - p.y) <= PAD) {
          if (Math.hypot(x - 52, y - p.y) <= PAD + 4)
            return { kind: "handle" as const, objectId: obj.id, handle: "anchor" as const };
          return { kind: "body" as const, objectId: obj.id };
        }
      }
      ...
      if (obj.type === "vline") {
        ...
        if (Math.abs(x - p.x) <= PAD) {
          if (Math.hypot(x - p.x, y - 40) <= PAD + 4)
            return { kind: "handle" as const, objectId: obj.id, handle: "anchor" as const };
          return { kind: "body" as const, objectId: obj.id };
        }
      }
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (draft && draft.p2) {
      ...
      setRenderTick((t) => t + 1); // ✅ FORÇA RE-RENDER DURANTE DRAG
      return;
    }
    ...
    if (drag.type === "move-handle") {
      ...
      setRenderTick((t) => t + 1); // ✅ FORÇA RE-RENDER
      return;
    }
      ...
      >
        ...
        >
          ...
          <StatCard
            ...
            valueColor={drawings.length ? ui.yellow : ui.red}
          />
          ...
          style={{
            ...
            cursor:
              currentTool === "cursor"
                ...
                : "crosshair",
            pointerEvents:
              currentTool === "cursor" && interactionMode === "navigate" ? "none" : "auto",
          }}
        >
          <svg width="100%" height="100%" style={{ overflow: "visible" }} key={`svg-${renderTick}`}>
            {drawings.map((obj) => {
              ...
              if (obj.type === "hline") {
                ...
                return (
                  <g key={`${obj.id}-${renderTick}`}>
                    ...
                    />
                    {isSelected ? (
                      <circle cx={52} cy={p.y} r={5} fill="#ffd95c" stroke="#06101d" strokeWidth={2} />
                    ) : null}
                  </g>
                );
              }
              ...
              if (obj.type === "vline") {
                ...
                return (
                  <g key={`${obj.id}-${renderTick}`}>
                    ...
                    />
                    {isSelected ? (
                      <circle cx={p.x} cy={40} r={5} fill="#ffd95c" stroke="#06101d" strokeWidth={2} />
                    ) : null}
                  </g>
                );
              }
...
export default function AtlasChartPro2() {
  ...
  const updateObject = (id: string, update: Partial<DrawingObject>) =>
    setDrawings((prev) =>
      prev.map((obj) => (obj.id === id ? ({ ...obj, ...update } as DrawingObject) : obj))
    );
