...
function ChartPanel({
  ...
  const interactiveTool = ["trendline", "hline", "vline", "ray"].includes(selectedTool);
  ...
  const hitHandle = (d: ChartDrawObject, x: number, y: number): "start" | "end" | null => {
    const r = 8;
    if (Math.hypot(x - d.x1, y - d.y1) <= r) return "start";
    if (d.type !== "hline" && d.type !== "vline" && Math.hypot(x - d.x2, y - d.y2) <= r) return "end";
    return null;
  };
  ...
  const hitDrawing = (x: number, y: number) => {
    for (let i = drawings.length - 1; i >= 0; i -= 1) {
      ...
      const handle = hitHandle(d, x, y);
      if (handle) return { id: d.id, kind: handle };
      if (d.type === "hline" && Math.abs(y - d.y1) < 8) return { id: d.id, kind: "move" as const };
      if (d.type === "vline" && Math.abs(x - d.x1) < 8) return { id: d.id, kind: "move" as const };
      const x2 = d.type === "ray" ? Math.max(d.x2, getCanvasSize().width) : d.x2;
      const y2 = d.type === "ray" ? d.y1 + ((d.y2 - d.y1) / Math.max(d.x2 - d.x1, 1)) * (x2 - d.x1) : d.y2;
      if (distToSegment(x, y, d.x1, d.y1, x2, y2) < 8) return { id: d.id, kind: "move" as const };
    }
    return null;
  };

  const beginDrawing = (p: { x: number; y: number; w: number; h: number }) => {
    const hit = hitDrawing(p.x, p.y);
      ...
      if (hit) {
        ...
        if (found && !found.locked) {
          if (hit.kind === "move") {
            ...
            setDragState({ id: hit.id, kind: hit.kind });
          }
        }
      } else {
    ...
    >
      ...
      >
        ...
        >
          ...
          {[
            ...
            ["Desenhos", String(drawings.filter((d) => !d.hidden).length), drawings.length ? ui.yellow : ui.red],
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          }}>{selectedDrawing?.locked ? "Destravar" : "Travar"}</TopButton>
          <TopButton onClick={() => {
            if (!selectedDrawingId) return;
            setDrawings((prev) => prev.map((d) => d.id === selectedDrawingId ? { ...d, hidden: !d.hidden } : d));
          }}>{selectedDrawing?.hidden ? "Mostrar" : "Ocultar"}</TopButton>
...
function FluxoModule({ events }: { events: ScannerEvent[] }) {({ events }: { events: ScannerEvent[] }) {
  return (
