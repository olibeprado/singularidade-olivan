...
export default function AtlasChartPro() {
  ...
  const panState = useRef<{
    isPanning: boolean;
    startLogicalRange: LogicalRange | null;
    startX: number | null;
    startY: number | null;
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    panState.current = {
      ...
      startX: e.clientX,
      startY: e.clientY,
      startPrice: chart.rightPriceScale().coordinateToPrice(e.clientY - rect.top) ?? null,
    };
  };
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (panState.current.isPanning && currentTool === "cursor") {
      ...
      if (dx !== 0) {
        ...
        timeScale.scrollToPosition(timeScale.scrollPosition() + dx, false);
      }

      if (dy !== 0) {
        const priceScale = chart.rightPriceScale();
        const currentPrice = priceScale.coordinateToPrice(rect.height / 2);
        if (currentPrice !== null) {
          const priceChangeAmount = (dy / 200) * (currentPrice * 0.02);
          priceScale.setPriceRange(currentPrice + priceChangeAmount, currentPrice - priceChangeAmount);
        }
      }
